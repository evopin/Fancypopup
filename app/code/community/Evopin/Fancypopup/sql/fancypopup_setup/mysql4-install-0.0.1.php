...
DROP TABLE IF EXISTS {$this->getTable('shopgo_featuredpopup')};
CREATE TABLE {$this->getTable('shopgo_featuredpopup')} (
...
DROP TABLE IF EXISTS `{$this->getTable('shopgo_featuredpopup_store')}`;
CREATE TABLE `{$this->getTable('shopgo_featuredpopup_store')}` (
  `featured_id` int(11) unsigned NOT NULL,
  `store_id` smallint(5) unsigned NOT NULL,
  PRIMARY KEY (`featured_id`,`store_id`),
  CONSTRAINT `FK_FEATUREDPOPUP_STORE_FEATUREDPOPUP` FOREIGN KEY (`featured_id`) REFERENCES `{$this->getTable('shopgo_featuredpopup')}` (`featuredpopup_id`) ON UPDATE CASCADE ON DELETE CASCADE,
